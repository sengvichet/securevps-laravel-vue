...
<h1>חבילות אחסון אתרים שיתופיים - כשמוצר ותקציב נפגשים</h1>
</div>
</div>
</div>
...
<div class="pricingboxes">
    ...
    <?php var_dump($sharedList); exit;?>
    @foreach ($sharedList as $shared)
      @if($shared['show_on_web'])
        <div data-wow-delay="{{ $wowEffectDelay += 0.1 }}s"  class="small-12 large-3 medium-3 columns wow fadeInUp" style="float:right">
            ...
            <ul class="pricing-table">
                ...
                <li class="description"><strong> {{ $shared['foreign_name'] }}<br /><font size=2>{{ $shared['id'] }}</font></strong></li>
                ...
                <li class="bullet-item"> BasicEmail <b style="font-size:18px"> {{ filterWebspaceMB($shared['itemprops']['BasicEmail']) }} </b> </li>
                <li class="bullet-item"> BasicDatabase <b style="font-size:18px"> {{ filterWebspaceMB($shared['itemprops']['BasicDatabase']) }} </b> </li>
                <li class="bullet-item"> BasicSubDomain <b style="font-size:18px"> {{ filterWebspaceMB($shared['itemprops']['BasicSubDomain']) }} </b> </li>
                <li class="bullet-item"> BasicFTP <b style="font-size:18px"> {{ filterWebspaceMB($shared['itemprops']['BasicFTP']) }} </b> </li>
                <li class="bullet-item"> BasicDomain <b style="font-size:18px"> {{ filterWebspaceMB($shared['itemprops']['BasicDomain']) }} </b> </li>
